...
const sliderItemHeight = sliderItems[0].offsetHeight; // single item height
console.dir(sliderItemHeight);
const sliderItemWidth = sliderItems[0].offsetWidth; // single Item width
const totalItems = sliderItems.length;
const lastItemsIndex = sliderItems.length - 1;

const nextBtn = document.querySelector('.top');
const prevBtn = document.querySelector('.bottom');
...
prevBtn.addEventListener('click', (event) => {
  ...
  slider.style.transform = `translateY(-${sliderItemHeight * currentIndex}px)`;
});
